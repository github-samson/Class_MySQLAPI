...
; Namespace:   MySQLAPI
; AHK version: v2.0-beta.3+
; Author:      panofish/just me
; Version:     2.0.00.00/2021-11-11/samsonwang     - Align with AHK v2 (using v2converter.ahk, and modify the compared differences manually)
;              1.0.01.00/2015-08-20/just me        - libmysql.dll error handling
...
Class MySQLAPI {
   ...
   ; ===================================================================================================================
   ; META FUNCTION __New
   ; Load and initialize libmariadb.dll which is supposed to be in the sript's folder.
   ; Parameters:    LibPath  - Optional: Absolute path of libmariadb.dll
   ; ===================================================================================================================
   __New(LibPath := "") {
      Static LibMySQL := A_ScriptDir . "\libmariadb.dll"
      ...
      ; Init MySQL
      If !(MYSQL := This.Init()) {
         MsgBox("Could not initialize MySQL!", "MySQL Error!", 16)
         Return False
      }
      This.MYSQL := MYSQL
      If (This.Options("MYSQL_SET_CHARSET_NAME", "utf8") != MySQLAPI.MySQL_SUCCESS) {
         MsgBox("Set option MYSQL_SET_CHARSET_NAME failed!", "MySQL Error!", 16)
         Return False
      }
      If (This.Options("MYSQL_OPT_RECONNECT", True) != MySQLAPI.MySQL_SUCCESS) {
         MsgBox("Set option MYSQL_OPT_RECONNECT failed!", "MySQL Error!", 16)
         Return False
      }
   ...
   GetField(&MYSQL_FIELD) {
      ...
      Field.Type      := MySQLAPI.FIELD_TYPE.%NumGet(MYSQL_FIELD + 0, Offset, "UInt")%
      Return Field
   }
   ; ===================================================================================================================
      ...
      If (MYSQL_ROW := This.Fetch_Row(MYSQL_RES)) {
         ...
         Loop This.Num_Fields(MYSQL_RES) {
            ...
            If (Len := NumGet(Lengths + 0, 4 * J, "UInt"))
               Row.InsertAt(A_index, StrGet(NumGet(MYSQL_ROW + 0, A_PtrSize * J, "UPtr"), Len, "UTF-8") )
            Else
               Row.InsertAt(A_index, "" )
         }
         Return Row
      }
      Return False
   ...
   GetResult() {
      ...
      While(MYSQL_FIELD := This.Fetch_Field(MYSQL_RES))
         Result.Fields.InsertAt(A_Index, This.GetField(&MYSQL_FIELD) )
      While (Row := This.GetNextRow(MYSQL_RES))
         Result.Records.InsertAt(A_index, Row )
   ...
   ; ===================================================================================================================
   UTF8(&Str) {
      Var := Str, Str := Buffer(StrPut(Var, "UTF-8"), 0), StrPut(Var, Str, "UTF-8")
      Return Str
   }
   ; ===================================================================================================================
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Affected_Rows() {
      Return DllCall("libmariadb.dll\mysql_affected_rows", "Ptr", This.MYSQL, "UInt64")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   AutoCommit(Mode) {
      Return DllCall("libmariadb.dll\mysql_autocommit", "Ptr", This.MYSQL, "Char", Mode, "Char")
   }
   ; ===================================================================================================================
   ...
   Change_User(User, PassWd, DB) {
      Return DllCall("libmariadb.dll\mysql_change_user", "Ptr", This.MYSQL, "Ptr", This.UTF8(&User)
   ...
   ; ===================================================================================================================
   Character_Set_Name() {
      Return ((P := DllCall("libmariadb.dll\mysql_character_set_name", "Ptr", This.MYSQL, "UPtr")) ? StrGet(P, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Close() {
      DllCall("libmariadb.dll\mysql_close", "Ptr", This.MYSQL)
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Commit() {
      Return DllCall("libmariadb.dll\mysql_commit", "Ptr", This.MYSQL, "Char")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Create_DB(DB) {
      Return DllCall("libmariadb.dll\mysql_create_db", "Ptr", This.MYSQL, "Ptr", This.UTF8(&DB), "Int")
   }
   ; ===================================================================================================================
   ...
   Data_Seek(MYSQL_RES, Offset) {
      DllCall("libmariadb.dll\mysql_data_seek", "Ptr", MYSQL_RES, "UInt64", Offset)
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Drop_DB(DB) {
      Return DllCall("libmariadb.dll\mysql_drop_db", "Ptr", This.MYSQL, "Ptr", This.UTF8(&DB), "Int")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   EOF(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_eof", "Ptr", MYSQL_RES, "Char")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   ErrNo() {
      Return DllCall("libmariadb.dll\mysql_errno", "Ptr", This.MYSQL, "UInt")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Error() {
      Return ((S := DllCall("libmariadb.dll\mysql_error", "Ptr", This.MYSQL, "UPtr")) ? StrGet(S, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   Fetch_Field(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_fetch_field", "Ptr", MYSQL_RES, "UPtr")
   }
   ; ===================================================================================================================
   ...
   Fetch_Field_Direct(MYSQL_RES, FieldNr) {
      Return DllCall("libmariadb.dll\mysql_fetch_field_direct", "Ptr", MYSQL_RES, "UInt", FieldNr, "UPtr")
   }
   ; ===================================================================================================================
   ...
   Fetch_Fields(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_fetch_fields", "Ptr", MYSQL_RES, "UPtr")
   }
   ; ===================================================================================================================
   ...
   Fetch_Lengths(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_fetch_lengths", "Ptr", MYSQL_RES, "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Fetch_Row(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_fetch_row", "Ptr", MYSQL_RES, "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Field_Count() {
      Return DllCall("libmariadb.dll\mysql_field_count", "Ptr", This.MYSQL, "UInt")
   }
   ; ===================================================================================================================
   ...
   Field_Seek(MYSQL_RES, Offset) {
      Return DllCall("libmariadb.dll\mysql_field_seek", "Ptr", MYSQL_RES, "UInt", Offset, "UInt")
   }
   ; ===================================================================================================================
   ...
   Field_Tell(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_field_tell", "Ptr", MYSQL_RES, "UInt")
   }
   ; ===================================================================================================================
   ...
   Free_Result(MYSQL_RES) {
      DllCall("libmariadb.dll\mysql_free_result", "Ptr", MYSQL_RES)
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Get_Client_Info() {
      Return ((S := DllCall("libmariadb.dll\mysql_get_client_info", "UPtr")) ? StrGet(S, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Get_Client_Version() {
      Return DllCall("libmariadb.dll\mysql_get_client_version", "Int")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Get_Host_Info() {
      Return ((P := DllCall("libmariadb.dll\mysql_get_host_info", "Ptr", This.MYSQL, "UPtr")) ? StrGet(P, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Get_Proto_Info() {
      Return DllCall("libmariadb.dll\mysql_get_proto_info", "Ptr", This.MYSQL, "UInt")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Get_Server_Info() {
      Return ((P := DllCall("libmariadb.dll\mysql_get_server_info", "Ptr", This.MYSQL, "UPtr")) ? StrGet(P, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Get_Server_Version() {
      Return DllCall("libmariadb.dll\mysql_get_server_version", "Ptr", This.MYSQL, "UInt")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Info() {
      Return ((S := DllCall("libmariadb.dll\mysql_info", "Ptr", This.MYSQL, "UPtr")) ? StrGet(S, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Init(MYSQL := 0) {
      Return DllCall("libmariadb.dll\mysql_init", "Ptr", MYSQL, "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Insert_ID() {
      Return DllCall("libmariadb.dll\mysql_insert_id", "Ptr", This.MYSQL, "UInt64")
   }
   ; ===================================================================================================================
   ...
   List_Fields(Table, Like := "") {
      Return DllCall("libmariadb.dll\mysql_list_fields", "Ptr", This.MYSQL, "Ptr", This.UTF8(&Table)
   ...
   ; ===================================================================================================================
   List_Tables(Like := "") {
      Return DllCall("libmariadb.dll\mysql_list_tables", "Ptr", This.MYSQL
                   , "Ptr", (Like = "" ? 0 : This.UTF8(&Like)), "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   More_Results() {
      Return DllCall("libmariadb.dll\mysql_more_results", "Ptr", This.MYSQL, "Char")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Next_Result() {
      Return DllCall("libmariadb.dll\mysql_next_result", "Ptr", This.MYSQL, "Int")
   }
   ; ===================================================================================================================
   ...
   Num_Fields(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_num_fields", "Ptr", MYSQL_RES, "UInt")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Num_Rows(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_num_rows", "Ptr", MYSQL_RES, "UInt64")
   }
   ; ===================================================================================================================
   ...
   Options(Option, Arg) {
      ...
      if !isInteger(Option)
         intOption := MYSQL_Option.%Option%
	  else
	     intOption := Option
	  ; MsgBox "Return DllCall(`"libmariadb.dll\mysql_options`", `"Ptr`"," . This.MYSQL . ", `"Int`", " . intOption . ", `"Int64P`", `&" . Arg . ", `"Int`")"
      if isInteger(Arg)
         Return DllCall("libmariadb.dll\mysql_options", "Ptr", This.MYSQL, "Int", intOption, "Int64P", &Arg, "Int")
      Return DllCall("libmariadb.dll\mysql_options", "Ptr", This.MYSQL, "Int", intOption, "Ptr", This.UTF8(&Arg), "Int")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Ping() {
      Return DllCall("libmariadb.dll\mysql_ping", "Ptr", This.MYSQL, "Int")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Query(SQL) {
      Return DllCall("libmariadb.dll\mysql_query", "Ptr", This.MYSQL, "Ptr", This.UTF8(&SQL), "Int")
   }
   ; ===================================================================================================================
   ...
   Real_Connect(Host, User, PassWd, DB := "", Port := 3306, Socket := 0, Flags := 0) {
      ...
      Else
         PtrDB := This.UTF8(&DB)
      If !(MYSQL := DllCall("libmariadb.dll\mysql_real_connect", "Ptr", This.MYSQL, "Ptr", This.UTF8(&Host)
   ...
   Real_Escape_String(&From) {
      ...
      SO := Buffer((L * 2) + 1, 0)
      N := DllCall("libmariadb.dll\mysql_real_escape_string", "Ptr", This.MYSQL, "Ptr", SO, "Ptr", SI, "UInt", L, "UInt")
   ...
   Real_Query(&SQL, Length) {
      Return DllCall("libmariadb.dll\mysql_real_query", "Ptr", This.MYSQL, "Ptr", &SQL, "UInt", Length, "Int")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Rollback() {
      Return DllCall("libmariadb.dll\mysql_rollback", "Ptr", This.MYSQL, "Char")
   }
   ; ===================================================================================================================
   ...
   Row_Seek(MYSQL_RES, Offset) {
      Return DllCall("libmariadb.dll\mysql_row_seek", "Ptr", MYSQL_RES, "Ptr", Offset, "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Row_Tell(MYSQL_RES) {
      Return DllCall("libmariadb.dll\mysql_row_tell", "Ptr", MYSQL_RES, "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Select_DB(DB) {
      Return DllCall("libmariadb.dll\mysql_select_db", "Ptr", This.MYSQL, "Ptr", This.UTF8(&DB), "Int")
   }
   ; ===================================================================================================================
   ...
   Set_Character_Set(CSName) {
      Return DllCall("libmariadb.dll\mysql_set_character_set", "Ptr", This.MYSQL, "Ptr", This.UTF8(&CSName), "Int")
   }
   ; ===================================================================================================================
   ...
   Set_Server_Option(Option) {
      Return DllCall("libmariadb.dll\mysql_set_server_option", "Ptr", This.MYSQL, "Int", Option, "Int")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   SQLState() {
      Return ((P := DllCall("libmariadb.dll\mysql_sqlstate", "Ptr", This.MYSQL, "UPtr")) ? StrGet(P, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Stat() {
      Return ((P := DllCall("libmariadb.dll\mysql_stat", "Ptr", This.MYSQL, "UPtr")) ? StrGet(P, "UTF-8") : "")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Store_Result() {
      Return DllCall("libmariadb.dll\mysql_store_result", "Ptr", This.MYSQL, "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Thread_ID() {
      Return DllCall("libmariadb.dll\mysql_thread_id", "Ptr", This.MYSQL, "UInt")
   }
   ; ===================================================================================================================
   ; After invoking mysql_query() or mysql_real_query(), you must call mysql_store_result() or mysql_use_result() for
   ; every statement that successfully produces a result set (SELECT, SHOW, DESCRIBE, EXPLAIN, CHECK TABLE, and so
   ...
   ; ===================================================================================================================
   Use_Result() {
      Return DllCall("libmariadb.dll\mysql_use_result", "Ptr", This.MYSQL, "UPtr")
   }
   ; ===================================================================================================================
   ...
   ; ===================================================================================================================
   Warning_Count() {
      Return DllCall("libmariadb.dll\mysql_warning_count", "Ptr", This.MYSQL, "UInt")
   }
}
